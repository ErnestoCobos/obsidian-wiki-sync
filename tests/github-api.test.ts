...
describe('GitHub API Interactions', () => {
  ...
  describe('getWikiPageContent', () => {
    ...
    it('should handle directory response instead of file', async () => {
      ...
      // Should throw error
      await expect(plugin.getWikiPageContent('Directory')).rejects.toThrow('Expected a file but got a directory');
    });
    ...
    it('should handle empty content', async () => {
      ...
      // Should throw error
      await expect(plugin.getWikiPageContent('EmptyFile')).rejects.toThrow('Response does not contain content');
    });
  ...
  describe('updateWikiPage', () => {
    ...
    it('should handle directory response instead of file', async () => {
      ...
      // Should throw error
      await expect(plugin.updateWikiPage('Directory', 'New content')).rejects.toThrow('Expected a file but got a directory');
    });
    ...
    it('should handle response without SHA', async () => {
      ...
      // Should throw error
      await expect(plugin.updateWikiPage('NoSha', 'New content')).rejects.toThrow('Response does not contain sha');
    });
    
    it('should handle API errors during update', async () => {
      // This will trigger an error in our mock
      await expect(plugin.updateWikiPage('ErrorFile', 'Error content')).rejects.toThrow('Failed to create/update file');
    });
